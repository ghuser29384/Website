...
flowchart LR
    ...
    C --> T
    T --> M[Coverage and promotion matrix\nsource-completeness + license gates + release coverage summary]
    ...
    D --> I
    I --> G[Search index\nplace names, aliases, ambiguity metadata, coverage status]
    G --> E
    D --> UX[UX contract and evidence-disclosure templates\ncoverage badges + issue cards + comparability warnings + audit drawers + no-data states]
    UX --> E
    UX --> G
...
|---|---|---|---:|---|
| P0 | **Coverage-status model and UI honesty pass** | Every place page and map selection preserves the release `coverage_status` vocabulary from the generated place-index schemas; at minimum support `adm1_context_overlay`, `boundary_only`, the current canonical country-profile status, and `no_data`, with separate human-readable labels if copy changes; place rows are release-scoped because `coverage_status`, `coverage_reason`, `missing_inputs`, parent hierarchy, and boundary-source metadata can change across immutable releases; coverage badges are present in map legend, search results, place pages, and `/places/` listings; when canonical coverage remains sparse or when the generated default-ranking-readiness rule has not passed, the homepage and global atlas default to coverage/availability status rather than pain rankings; the coverage module exposes canonical place profiles, canonical country profiles, release measurements, direct place-evidence rows, ADM1 context rows, pre-rendered ADM1 pages, known sparse areas, and a release-level coverage summary used by homepage copy, sparse-coverage notices, regression tests, and default-ranking-readiness gates | 26 | Low |
| P0 | **Place-profile UX hierarchy and evidence-disclosure pattern** | Every place page, map selection, search result, and compare row uses the same user-facing hierarchy: release/snapshot badge, coverage badge, top source-of-pain cards, evidence-kind chip, affected-population label, source-vintage/reference-period line, rank-stability or uncertainty chip where applicable, short caveat, correction link, and an expandable audit drawer for sources, raw snapshots, assumptions, method recipe, downloads, and provenance; boundary-only and no-data pages show “what is known,” “what is missing,” and “what would promote this place” instead of empty or misleading issue cards; compare views and top-source rankings must show a not-directly-comparable state unless compared rows share compatible registered layer, unit, ranking-mode, reference-period, and evidence-kind semantics; narrow-screen and low-bandwidth layouts preserve the same evidence order and offer a list-first route when the map is collapsed, unavailable, or too costly to load | 28 | Medium |
| P0 | **Homepage/search task-flow and evidence glossary** | The homepage and `/places/` route expose three plain-language entry tasks—find a place, compare places, and download/cite data—while showing generated current-release coverage counts, current release, coverage depth, sparse-coverage caveat, and default-ranking-readiness state before any ranking-like claim; search results show place level, parent place, alias type, local/common name where available, coverage status, and ambiguity/disambiguation text before selection; ambiguous aliases cannot auto-select without user confirmation; every coverage badge, evidence-kind chip, uncertainty/rank-stability chip, source-vintage label, reference-period label, and comparability warning resolves to generated plain-language glossary copy and a stable help URL or tooltip ID | 18 | Low |
...
| P0 | **Layer registry and rendering contract** | The release compiler emits `/v1/layers.json` from a typed layer registry; every measurement, table row, map legend item, compare view, and OGC feature references a registered `layer_id`; each layer declares evidence kind, value type, unit label, ranking mode, comparability group, evidence-compatibility rule, allowed source groups, assumption requirements where applicable, color/legend semantics, no-data style, colorblind-safe palette or non-color encoding where needed, and map/table rendering semantics so proxy, priority-overlay, modeled, direct, and boundary layers cannot be styled, compared, or ranked interchangeably; every JSON-array layer reference such as issue `linked_layer_ids` and method `input_layer_ids` resolves to same-release layer rows | 22 | Low |
...
| P0 | **Release compiler and typed contracts** | One build command produces validated JSON/CSV/GeoJSON/manifest outputs, with JSON Schema checks and checksum generation; `verify-release.ts` runs the full release gate suite and fails if any generated registry, contract, checksum, route, mode, attribution, UX/glossary, accessibility-smoke, or coverage invariant is missing | 36 | Medium |
...
| P0 | **Accessibility hardening** | Keyboard navigation for combobox, tablist, tables, compare flows, map legend controls, audit drawers, and map fallback verified in Playwright; reduced-motion support added; color is never the only channel for evidence kind, comparability, missingness, or uncertainty | 26 | Medium |
...
| P2 | **Deferred edge live-overlay API** | Do not build new edge overlay endpoints until current-release replay, coverage expansion, UX/evidence disclosure, and privacy-safe telemetry are stable and a separate no-sunk-cost review shows that live context helps a visible user task. If implemented, overlays stay off by default, require explicit user action, validate country-level place IDs, cache only successful normalized upstream responses, attach provenance labels, preserve snapshot-mode replay, and never overwrite release rows; live values cannot appear in default rankings while canonical coverage is sparse | 32 | Medium |
...
| P2 | **Scheduled source-update adapter framework** | Adapters run on declared source-review cadences, detect upstream checksum or schema changes, and open release-candidate PRs; they must not create user-facing realtime claims or bypass immutable release review | 32 | Medium |
...
|---|---|---|---|
| 0 | **Contract freeze and current-release replay** | Snapshot the current public URLs, route list, release IDs, coverage counts, artifact checksums, canonical place/profile counts, map default state, visible legends, and sparse-coverage notices; prove the current `2026-05-31.atlas.2` release can be rebuilt or mirrored without changing public behavior | Do not refactor UI behavior, add PMTiles, change coverage labels, or introduce ranking defaults while canonical coverage is sparse |
| 1 | **Release compiler core and registry generation** | Generate only the source/license, source-snapshot, provenance, artifact/distribution, boundary/geometry, layer, pain-source issue, method/transform, assumption, performance, release-mode, UX, and smoke-test registry fields needed to replay the current release and visible evidence-disclosure contract; existing public JSON/CSV/GeoJSON/OpenAPI/DCAT outputs remain byte-identical unless the diff is deliberate | Do not expand coverage, introduce new ranking methods, or keep speculative registry fields that are not used by a visible route, release artifact, or release-blocking test |
...
| 3 | **Frontend modularization with visual, accessibility, and UX-contract parity** | Split the monolithic frontend into typed modules while preserving existing URLs, search behavior, search-disambiguation states, map fallbacks, table equivalents, list-first narrow-screen flows, release-mode switching, coverage-first default state where needed, accessible map legends, coverage badges, issue-card hierarchy, audit drawers, no-data states, and smoke/a11y/UX contracts | Do not change the map stack, introduce new live overlay behavior, or redesign the visual language in a way that hides coverage/evidence status or implies unavailable precision |
...
| 5 | **Privacy-safe telemetry and live-overlay deferral** | Add privacy-safe telemetry only after snapshot-mode behavior is stable and the public analytics vocabulary/privacy gates pass; preserve existing labeled live-overlay UI only if required for current-public parity, but defer new edge overlay workers until a separate P2 review shows they support a visible user task | Do not let live overlay values overwrite canonical release rows, appear in default rankings, blur the snapshot/live distinction, or pull engineering effort away from release replay and coverage expansion |
...
**Contract consolidation rule:** treat named registries, packages, and tables as logical contracts, not as a requirement to create a separate package or table for every concept on day one. Start with the fewest executable modules that can replay the current release and enforce the visible contract. Split a registry into its own package or table only when it has an independent lifecycle, multiple consumers, materially different validation rules, or a clear owner boundary. If two registries always change together, merge them or generate them from one typed source.

**Complexity and deletion gate:** at the end of every cutline, run a no-sunk-cost review. Delete, merge, or defer any registry, table, package, endpoint, UX pattern, or test lane that does not materially support one of five things: current-release replay, coverage honesty, source/provenance/citation reproducibility, accessibility/list-first usability, or a visible user task. A feature should not survive merely because earlier drafts introduced it; it should survive only if removing it would probably weaken the atlas.

### UX/UI contract for place-first use

PainMap should not make users infer evidence quality from technical tables. The user-facing place profile should use a stable, repeated hierarchy on every place page, map selection, search result, and compare row:

1. **Identity and release state:** place name, admin level, parent place, snapshot/live badge, release ID, and coverage badge.
2. **What appears important here:** three to five source-of-pain cards only when the place has canonical evidence; when the place has ADM1 context evidence or other noncanonical context, the same component should be labeled as context rather than as a top ranking. Each card shows affected population, harm mechanism, evidence kind, reference period, source vintage, rank-stability or uncertainty chip, and a one-sentence caveat.
3. **Why it appears here:** a short explanation tying the card to registered layers, methods, assumptions, and source snapshots.
4. **What is missing:** for boundary-only, ADM1-context, or no-data states, show missing inputs, promotion blockers, and the next data step instead of empty rankings.
5. **Audit drawer:** expandable but always reachable source, raw-snapshot, method, assumption, sensitivity, provenance, download, and correction context.
6. **Comparison guardrail:** compare tables, map-selected comparisons, and “top source” rankings may only place values side by side without a warning when they share the same registered comparability group, layer, unit label, ranking mode, reference-period semantics, and evidence-kind compatibility rule. Otherwise, the UI must show a “not directly comparable” state and explain the mismatch rather than silently ranking unlike quantities.
7. **Entry task, coverage counts, and glossary layer:** the homepage and `/places/` route should make the three primary tasks obvious—find a place, compare places, and download/cite data—while showing generated current-release coverage counts such as canonical profiles, boundary-only places, ADM1 context rows, direct evidence rows, and the default-ranking-readiness state. Hero and search copy should describe the atlas as coverage-in-progress while canonical coverage is sparse. Every technical badge or chip should have generated plain-language help text. Terms such as “proxy,” “priority overlay,” “source vintage,” “reference period,” “rank stability,” “boundary only,” and “not directly comparable” should never appear without an adjacent tooltip, help link, or glossary definition.
8. **Search disambiguation and small-screen fallback:** search results should not silently choose among ambiguous aliases such as countries, regions, or historical/common names. Results should show place level, parent place, alias type, local/common name where available, coverage status, and ambiguity notes before selection. On narrow screens or slow connections, the same task flow should work through list-first cards and tables before the map loads.
9. **Map and legend semantics:** the global atlas should default to a coverage/availability view while canonical place coverage is sparse, with pain-ranking layers treated as explicit user-selected views. A pain-ranking layer may become the default only after the release coverage artifact declares that the project has passed a configured default-ranking-readiness threshold and the release notes explain why the default no longer overstates coverage. Every map legend must show layer name, unit, ranking mode, reference-period semantics, evidence kind, no-data style, uncertainty/hatching/opacity semantics, and whether values are directly comparable. Color should not be the only channel for evidence quality or missingness, and red/green or alarm-like palettes should not imply moral blame or precision that the data do not support.

This is a UX contract, not just copy guidance. The release compiler should emit stable UI contract rows, glossary definitions, tooltip/help IDs, coverage-first default requirements, default-ranking-readiness metadata, legend/color-scale fields, search-disambiguation metadata, list-first fallback requirements, or JSON fragments that the web app, smoke tests, and Playwright tests can consume. The UI may be visually simple, but it must not bury coverage status, evidence kind, reference period, source vintage, or caveat text behind specialist data-download pages or map-only interactions.
...
The package tree below is a logical decomposition, not a mandate to create every package immediately. In the first implementation, Codex may consolidate several registry packages into one `atlas-contracts` or `release-contracts` package if that produces a smaller, more testable system without weakening release replay or validation.
...
painmap/
  ...
  packages/
    ...
    atlas-core/
    release-compiler/
    atlas-contracts/              # optional consolidated home for early registry/UX/telemetry contracts
    source-adapters/
    source-registry/
    source-snapshots/
    provenance-registry/
    ...
    correction-workflow/
  workers/
    live-overlay/                 # optional/deferred; do not build until a P2 no-sunk-cost review justifies live context
  data/
    raw/
    compiled/
      releases/
  scripts/
    ...
    build-coverage-summary.ts
    build-ux-contracts.ts
    build-analytics.ts
...
create table source_snapshot (
  ...
  upstream_url text not null,
  retrieved_at timestamptz not null,           -- validator must ensure retrieved_at <= release.published_at
  source_vintage text not null,
  media_type text not null,
  checksum_algorithm text not null default 'sha256',
  checksum text not null,
...
create table layer (
  ...
  evidence_compatibility_rule text not null default 'same_evidence_kind' check (
    evidence_compatibility_rule in ('same_evidence_kind', 'registered_compatible_evidence', 'never_directly_comparable')
  ),
  source_ids jsonb not null default '[]'::jsonb, -- validator must resolve each array item to source(release_id, source_id)
  required_assumption_types jsonb not null default '[]'::jsonb,
  render_semantics jsonb not null default '{}'::jsonb,  -- legend class, color-scale/palette ID, non-color encoding, no-data style, hatch/outline rules, table formatting, map eligibility
  visible_in_atlas boolean not null default true,
...
create table ux_glossary_term (
  release_id text not null references release(release_id),
  term_id text not null,                  -- e.g. glossary.evidence.proxy
  term text not null,
  plain_language_label text not null,
  short_definition text not null,
  long_definition text,
  help_url text not null,
  related_registry_type text,
  related_registry_id text,
  primary key (release_id, term_id)
);

create table ux_surface_contract (
  release_id text not null references release(release_id),
  surface_id text not null,                -- e.g. ux.place_profile.header or ux.search_result.card
  route_pattern text not null,
  surface_type text not null check (
    surface_type in ('homepage', 'places_index', 'place_profile', 'map_selection', 'map_legend', 'coverage_overview', 'search_result', 'compare_row', 'data_download', 'audit_drawer', 'no_data_state', 'mobile_list')
  ),
  required_badges jsonb not null default '[]'::jsonb,
  required_disclosure_fields jsonb not null default '[]'::jsonb,
  required_legend_fields jsonb not null default '[]'::jsonb,
  required_glossary_term_ids jsonb not null default '[]'::jsonb, -- validator must resolve each array item to ux_glossary_term(release_id, term_id)
  audit_drawer_required boolean not null default false,
  no_data_state_required boolean not null default false,
  comparability_warning_required boolean not null default false,
  search_disambiguation_required boolean not null default false,
  list_first_fallback_required boolean not null default false,
  coverage_first_default_required boolean not null default false,
  default_view_mode text check (default_view_mode is null or default_view_mode in ('coverage', 'ranking', 'table', 'download')),
  default_view_rationale text,
  default_ranking_readiness_rule text,
  map_legend_required boolean not null default false,
  check (not coverage_first_default_required or default_view_mode = 'coverage'),
  check (default_view_mode <> 'ranking' or default_ranking_readiness_rule is not null),
  check (not map_legend_required or jsonb_array_length(required_legend_fields) > 0),
...
create table release_coverage_summary (
  release_id text primary key references release(release_id),
  total_places integer not null check (total_places >= 0),
  canonical_place_profiles integer not null check (canonical_place_profiles >= 0),
  canonical_country_profiles integer not null check (canonical_country_profiles >= 0),
  canonical_measurements integer not null check (canonical_measurements >= 0),
  direct_place_evidence_rows integer not null check (direct_place_evidence_rows >= 0),
  boundary_only_places integer not null check (boundary_only_places >= 0),
  adm1_context_rows integer not null check (adm1_context_rows >= 0),
  no_data_places integer not null check (no_data_places >= 0),
  pre_rendered_adm1_pages integer not null default 0 check (pre_rendered_adm1_pages >= 0),
  default_ranking_ready boolean not null default false,
  default_ranking_readiness_rule text not null,
  default_ranking_release_note_url text,
  summary_generated_at timestamptz not null,
  check (not default_ranking_ready or default_ranking_release_note_url is not null),
  check (canonical_place_profiles <= total_places),
  check (canonical_country_profiles <= total_places),
  check (boundary_only_places <= total_places),
  check (no_data_places <= total_places)
);
...
create table measurement (
  ...
  reference_period_semantics text not null check (
    reference_period_semantics in ('instant', 'annual', 'multi_year', 'source_defined', 'not_applicable')
  ),
  reference_period_start date,
  reference_period_end date,
  check (reference_period_start is null or reference_period_end is null or reference_period_start <= reference_period_end),
...
create table correction_case (
  ...
  check (status not in ('resolved', 'rejected', 'superseded') or resolved_at is not null),
  ...
  schemas:
    PlaceProfile:
      ...
      properties:
        ...
        release_coverage_summary:
          type: object
          description: Release-level coverage counts and default-ranking-readiness metadata used by homepage copy, map defaults, sparse-coverage notices, and coverage-regression tests.
          required: [total_places, canonical_place_profiles, canonical_country_profiles, canonical_measurements, direct_place_evidence_rows, boundary_only_places, adm1_context_rows, no_data_places, default_ranking_ready, default_ranking_readiness_rule, summary_generated_at]
          properties:
            total_places: { type: integer }
            canonical_place_profiles: { type: integer }
            canonical_country_profiles: { type: integer }
            canonical_measurements: { type: integer }
            direct_place_evidence_rows: { type: integer }
            boundary_only_places: { type: integer }
            adm1_context_rows: { type: integer }
            no_data_places: { type: integer }
            pre_rendered_adm1_pages: { type: integer }
            default_ranking_ready: { type: boolean }
            default_ranking_readiness_rule: { type: string }
            default_ranking_release_note_url:
              type: string
              description: Required by release validation when `default_ranking_ready` is true.
        ...
        release_citation:
          ...
          required: [recommended_citation, published_at, manifest_url, citation_file_url, citation_json_url]
          properties:
            recommended_citation: { type: string }
            published_at:
              type: string
              format: date-time
              description: Immutable release publication timestamp used to validate that raw source snapshots were retrieved before publication.
        ...
        source_snapshots:
          type: array
          description: Release-scoped raw-input snapshots referenced by this place profile's measurements; each entry includes enough checksum and retrieval metadata to reproduce source-file lineage without relying on mutable upstream URLs.
          items:
    ...
    SourceSnapshot:
      type: object
      required: [release_id, snapshot_id, source_id, upstream_url, retrieved_at, source_vintage, media_type, checksum_algorithm, checksum]
      properties:
        ...
        raw_artifact_id: { type: string }
        retrieval_metadata:
          type: object
          description: Non-secret retrieval context such as adapter version, request parameters, upstream ETag/Last-Modified values, and normalization notes needed for reproducibility.
    Measurement:
      type: object
      required:
        - claim_id
      ...
      properties:
        ...
        layer_metadata:
          ...
          properties:
            ...
            comparability_group_id: { type: string }
            evidence_compatibility_rule: { type: string }
        ...
        reference_period_semantics:
          type: string
          enum: [instant, annual, multi_year, source_defined, not_applicable]
          description: Machine-readable basis used by compare views to decide whether rows share compatible reference-period semantics.
        reference_period_start:
          type: string
          format: date
          description: Optional normalized start date when the source period can be represented as a date range.
        reference_period_end:
          type: string
          format: date
          description: Optional normalized end date when the source period can be represented as a date range.
        method_id: { type: string }
        method_version: { type: string }
        ...
        source_file_checksum:
          type: string
          description: Optional legacy or summary checksum for simple one-snapshot exports; prefer `source_snapshots` plus `source_snapshot_ids` for full lineage.
        source_file_checksum_algorithm:
          type: string
          description: Optional algorithm for `source_file_checksum`; omit when the measurement is backed by multiple raw snapshots and use per-snapshot checksum metadata instead.
...
#### Defer any new edge overlay adapter unless a separate review justifies live context

PainMap’s public contract already says live overlays are short-lived, exploratory, and separate from immutable release rows. Preserve that, but do not treat live overlays as a required next-stage feature. While canonical coverage is sparse, improving release replay, coverage expansion, place-page clarity, and download/citation workflows is more important than adding current-source query paths. If a later no-sunk-cost review shows that live context supports a visible user task, then move overlays behind a thin cache/provenance layer; do not let browser code freehand-fetch upstreams and merge them opaquely. Never let live values drive default rankings or immutable release artifacts. The worker sample below is a future optional pattern, not a P0/P1 implementation requirement. citeturn30view1turn25view4
...
Second, **accessibility tests**: Playwright should assert the keyboard and screen-reader paths for the combobox, release-mode tabs, compare flows, place tables, issue-card lists, audit drawers, no-data states, and map-independent evidence-disclosure paths, while manual review confirms behavior against the WAI APG combobox pattern and WCAG 2.2. PainMap already exposes the semantic pieces; now it must prove interaction fidelity. citeturn18view0turn18view1turn18view3turn26view0turn43search0turn43search1turn46search1
...
Tenth, **coverage and promotion-gate tests**: every release should produce a coverage diff and `release_coverage_summary` against the previous immutable release, including total places, canonical profiles, canonical measurements, direct evidence rows, demotions, boundary-only counts, ADM1 context counts, no-data counts, pre-rendered ADM1 pages, source-completeness decisions, and default-ranking-readiness state. CI should fail on unexplained canonical promotions, unexplained drops in profile or measurement counts, any stale or inconsistent release coverage summary, any default-ranking-ready flag that lacks a documented rule and release-note URL, any coverage summary without a generation timestamp, any cross-release place/coverage/status join, any `present` or `stale` `place_source_coverage` row that lacks source vintage or license ID, any place alias whose source reference is missing or cross-release, or any canonical profile whose `place_source_coverage` rows do not satisfy the documented promotion rule.

Eleventh, **layer registry and rendering-contract tests**: every measurement, map legend entry, compare row, table column, OGC feature property, and export field that references a `layer_id` should resolve to exactly one registered layer in `/v1/layers.json`. CI should fail if a layer is missing evidence kind, value type, unit label, ranking mode, comparability group, evidence-compatibility rule, source requirements, color/legend semantics, no-data style, or rendering semantics; if a measurement's evidence kind, value type, unit label, or ranking mode fails the same-release registered-layer semantic foreign key; if JSON-array layer references such as `linked_layer_ids` or `input_layer_ids` are orphaned or cross-release; if two surfaces render the same layer with contradictory labels, units, color scales, no-data styles, ranking modes, comparability groups, or evidence-compatibility rules; or if proxy, priority-overlay, modeled, direct, and boundary layers are visually, statistically, or comparatively merged without an explicit registered rule.
...
Twenty-second, **public smoke-manifest and deployment parity tests**: every public route, API endpoint, data artifact, OGC surface, release alias, downloadable artifact, and critical UI route should resolve to `/data/ui-smoke.json` or `/data/endpoint-smoke.json` as appropriate. CI and post-deploy checks should fail if generated smoke manifests disagree with deployed status codes, content types, canonical metadata, structured-data blocks, critical selectors, coverage/evidence badges, coverage-first map default state where required, map legends, audit-drawer triggers, no-data-state copy, ARIA landmarks, release aliases, required artifact IDs, or artifact checksums.

Twenty-third, **UX contract and evidence-disclosure tests**: every homepage task entry, place page, map selection, search result, compare row, audit drawer, no-data state, and narrow-screen/list-first flow should resolve to generated `ux_surface_contract` rows and `ux_glossary_term` rows for release badge, coverage badge, evidence-kind chip, affected-population label, reference-period/source-vintage line, rank-stability or uncertainty chip, caveat text, correction link, glossary/help text, search-disambiguation text, and audit-drawer affordance. CI should fail if the homepage hides release, generated coverage counts, or sparse-coverage status before ranking-like claims, if the global atlas defaults to a pain-ranking view while canonical coverage is sparse or before the generated default-ranking-readiness rule passes, if canonical rows or context rows render without clearly different evidence status, if boundary-only/no-data places or noncanonical context places render ranking-like issue cards instead of missing-input/context explanations, if a map legend omits layer name, unit, ranking mode, reference-period semantics, evidence kind, no-data style, uncertainty encoding, or comparability status, if color is the only channel for missingness/uncertainty/evidence quality, if a technical badge/chip lacks generated plain-language glossary copy, if an ambiguous search alias auto-selects without a disambiguation state, if the narrow-screen flow requires the map before list/table content is usable, if a top issue card cannot be traced to measurement rows and registered issue/layer/method records, if search and compare surfaces use different badge/copy semantics from the place page, or if compare/top-source UI ranks values together without a registered same-comparability-group/layer/unit/ranking-mode/reference-period-semantics/evidence-kind compatibility rule.
...
| UX evidence-disclosure, glossary, map-legend, and comparability integrity | 100% homepage task entries, place pages, map selections, map legends, search results, compare rows, and narrow-screen/list-first flows resolve to `ux_surface_contract` rows and expose the same release, coverage, evidence-kind, reference-period, source-vintage, caveat, correction, glossary/help, map-legend, and audit-drawer affordances; 0 default pain-ranking views while canonical coverage is sparse; 0 empty or ranking-like cards for boundary-only, no-data, or noncanonical context places; 0 unexplained technical badges/chips; 0 ambiguous search aliases that auto-select without disambiguation; 0 compare/top-source rankings without a registered same-layer/unit/ranking-mode/reference-period/evidence-kind compatibility rule | Prevents the atlas from being technically auditable but confusing or misleading in ordinary use |
| Release build success | 100% | No partial artifact publication |
| Complexity/deletion review completion | 100% of cutlines and release candidates include a no-sunk-cost consolidation/deletion note | Prevents registry, package, endpoint, and UX-pattern sprawl from path dependence |
...
| Source, license, source-snapshot, and attribution resolution | 100% | Every source, source snapshot, measurement source vintage, measurement reference period, measurement reference-period semantics, measurement display value, measurement unit label, source review cadence, positive cadence-days value where applicable, coherent last/next review date, license, attribution, redistribution field, scalar source/license/snapshot reference, and JSON-array source reference resolves from the same-release generated registry before publication; 0 orphaned JSON source or snapshot references; 0 cross-release source/license/snapshot joins; 0 source snapshots retrieved after `release.published_at` |
...
| Search success and disambiguation rate | > 90% for place-search sessions, with 0 silent ambiguous-alias auto-selections | Indicates naming/alias quality and prevents wrong-place selections |
...
| Coverage transparency and default-view integrity | 100% place pages show status; 100% homepage/search surfaces show generated release coverage counts; global atlas defaults to coverage until a generated default-ranking-readiness rule passes and a release-note URL justifies it | Prevents false completeness and premature ranking defaults |
...
| Claim registry and correction workflow integrity | 100% visible claim surfaces across measurement, layer, issue, boundary, source/source-snapshot/license, assumption, method, sensitivity, release-mode, citation, route, and artifact surfaces expose stable claim IDs and prefilled correction links; 0 duplicate claim IDs; 0 unknown or orphaned claim subject IDs; 0 cross-release correction-case/claim joins; 0 correction-case timestamp/status inconsistencies; 0 accepted corrections missing from registry, release diff, or changelog | Makes source disputes and claim corrections operational rather than merely nominal |
| Citation and archival integrity | 100% immutable releases emit internally consistent citation metadata, publication timestamps, citation JSON URLs, and manifest URLs; 0 archived artifact checksum mismatches | Makes reuse citeable beyond mutable site URLs and protects reproducibility if hosting changes |
...
| Evidence-disclosure and comparability usability | Place pages, map selections, search results, compare rows, and narrow-screen/list-first flows must expose coverage status, evidence kind, source vintage, reference period, uncertainty/rank-stability, caveat, correction path, and audit drawer in the same user-facing order, and compare/top-source rankings must refuse silent side-by-side ranking when registered layer, unit, ranking-mode, reference-period, or evidence-kind compatibility does not match |
| Layer semantics clarity | Every visible layer must preserve its registered evidence kind, value type, unit label, ranking mode, uncertainty/rendering semantics, color/legend semantics, no-data style, and same-release JSON-array layer references across map, table, compare, API, and export surfaces |
| Map and legend restraint | The default atlas should not present sparse canonical coverage as a global pain ranking; colors, hatching, opacity, and legends should communicate coverage, evidence kind, missingness, uncertainty, and comparability without implying blame or precision that the data do not support |
...
| Source and license clarity | Every exported value, boundary, layer, model card, citation artifact, release mode, method transform, assumption record, and provenance edge must resolve scalar and JSON-array source/license/snapshot references to same-release registered source, raw-input snapshot checksum/retrieval metadata, measurement source-vintage, measurement reference-period, measurement display-value, measurement unit-label, license, attribution, review-cadence, last/next-review, and redistribution metadata rather than relying on free-text references |
...
| Human-vs-animal comparability | Keep separate evidence labels, method notes, comparability groups, evidence-compatibility rules, and comparison warnings; never imply a settled universal conversion or rank human and animal harms together without a registered comparability rule |
...
| Place identity clarity | Official, local, code, transliteration, and common-name aliases should resolve to stable same-release place IDs, level-consistent parent/code relationships, and source-traced alias rows; ambiguous aliases should show place level, parent, alias type, coverage status, and disambiguation text rather than silently selecting the wrong place |
...
| Coverage honesty | Never let sparse coverage masquerade as global completeness; homepage/search copy must show generated release coverage counts, and coverage status, reason codes, missing-input fields, release coverage summary, and default ranking-readiness state, summary generation timestamp, and ranking-readiness release-note URL must belong to the same immutable release as the visible place row |
| Plain-language evidence disclosure | Every user-facing coverage label, evidence-kind chip, uncertainty/rank-stability chip, source-vintage label, reference-period label, comparability warning, and ambiguous-search warning should resolve to plain-language glossary/help copy so non-specialist users are not forced to infer meaning from technical terms |
| No invented completeness | A country or ADM1 cannot be promoted from boundary/context status to canonical profile unless same-release source-completeness, license, and QA thresholds pass; otherwise expose same-release `coverage_reason` and `missing_inputs`, and label any noncanonical issue cards as context rather than rankings |
...
| Correction channel | Every claim surface has a stable claim ID whose subject type and subject ID resolve to a same-release registry row, and links to an issue-reporting workflow with release, route, subject type, subject id, and provenance context; accepted corrections appear in a public correction registry and release/changelog diff |
...
> **Do not replace PainMap’s product philosophy.** Preserve immutable releases, provenance-first communication, and the equal-area world-atlas default. Implement the P0 cutlines in order, with a no-sunk-cost complexity/deletion review at the end of every cutline: first freeze and replay the current release contract, then refactor the site into typed modules and a release compiler, make coverage status, release coverage summaries, and default-ranking-readiness metadata explicit everywhere, make source/license registries, source-snapshot lockfiles, provenance registry exports, boundary/geometry metadata, artifact/distribution metadata, layer definitions, pain-source taxonomy and issue-card metadata, method/transform recipes, welfare/priority assumptions, and sensitivity scenarios versioned release artifacts, generate release citation/archive metadata, canonical route/sitemap/crawl-policy artifacts, release-scoped place identity/alias metadata, UX/evidence-disclosure templates and plain-language glossary entries for homepage task flows, coverage-first map defaults where needed, accessible map legends, search disambiguation states, place profiles, issue/context cards, comparability warnings, audit drawers, no-data states, and narrow-screen/list-first flows, privacy-preserving telemetry vocabulary, performance/cache budget artifacts, release-mode/overlay-isolation artifacts, public smoke-test manifests, and stable claim registry plus claim-level correction workflow metadata from the same release inventory, expand canonical country proxy profiles only where source-completeness and license checks pass, then add a lazy detail-map layer using MapLibre + PMTiles for deeper exploration. Treat live overlays as deferred, explicit-user-action context—not canonical truth—and do not build new edge overlay workers unless a separate review shows they serve a visible task; consolidate logical registries/packages unless splitting them materially improves replay, validation, or ownership. Block merges through `npm run verify:release` on schema validation, provenance-registry integrity, boundary/geometry registry integrity, artifact/distribution integrity, same-release source/license/source-snapshot attribution, source-vintage, reference-period, display-value, unit-label, review-cadence, and JSON-reference integrity, same-release layer/assumption/artifact reference, checksum-algorithm, cache-policy, and artifact-license integrity, layer-registry/semantic-FK/rendering-contract/color-legend/comparability integrity, pain-source taxonomy/issue-card integrity, method-id/method-version/transform-version reproducibility, assumption traceability, sensitivity coverage, citation/archive integrity, route/crawlability integrity, telemetry privacy integrity, performance/cache-budget integrity, release-mode/overlay-isolation integrity, smoke-manifest deployment parity, release-scoped place/coverage integrity, place-identity/alias-search integrity, claim-registry/correction-link integrity, UX/evidence-disclosure, glossary, homepage task-flow, coverage-first default, release coverage-summary, ranking-readiness release-note, default-ranking-readiness, map-legend, search-disambiguation, list-first responsive-flow, and comparability integrity, accessibility, and geospatial integrity.
